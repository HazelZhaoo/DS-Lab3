...
// Copy constructor
Matrix::Matrix(const Matrix& copy_m): head(nullptr), num_cols(0), num_rows(0)
{
    copyFrom(copy_m);
}
...
void Matrix::copyFrom(const Matrix& obj)
{
    if (obj.head == nullptr)//check if matrix is empty
    {
        head = nullptr;
        num_rows = 0;
        num_cols = 0;
    ...
    else if (head->next_col == nullptr && head->next_row == nullptr)
    {
        head = obj.head;
        num_rows = 1;
        num_cols = 1;
